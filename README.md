...
👉 **[Insert Your GitHub Pages Hosted Link Here]**
...
4.  **Important Folder Rules:**
    *   The folder on the SD card must have the **exact same name** as your `.eldrum` file (e.g., if the file is `Pizzac.eldrum`, the folder must be named `Pizzac`).
...
-   **Format Transcoding:** Drag and drop audio in almost any format. The browser automatically transcodes them into standard 16-bit, 48 kHz, Stereo LPCM WAV files for native, low-latency playback [3.4.4].
...
-   **Dynamic Level Readouts:** Balances volume levels visually across all 8 slots. Adjusting a slot's velocity immediately updates its estimated peak dB and perceived LUFS loudness.
...
-   **Cumulative Footprint Estimate:** Displays an estimated total size of the final exported `.zip` file in real-time next to the Export button, assisting with hardware project RAM management.
